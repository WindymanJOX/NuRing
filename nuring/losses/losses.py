...
def angle_smoothness_loss(radius: torch.Tensor) -> torch.Tensor:
    diff = torch.abs(torch.roll(radius, shifts=-1, dims=1) - radius)
    return diff.mean()
...
class NuRingLoss(nn.Module):
    ...
    def forward(self, pred: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        mask_loss = bce + dsc
        radius_loss = weighted_smooth_l1(radius, y_radius, y_conf)
        smooth_loss = angle_smoothness_loss(radius)
